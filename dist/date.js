...
export function waktuRelative(from, to = new Date()) {
...
export function diffTanggal(from, to, unit = 'hari') {
    ...
    if (unit === 'hari')
        return diffMs >= 0 ? absDiffDays : -absDiffDays;
    if (unit === 'bulan')
        return diffMs >= 0 ? monthDiff : -monthDiff;
    if (unit === 'tahun')
        return diffMs >= 0 ? yearDiff : -yearDiff;
    // auto
    if (yearDiff >= 1)
